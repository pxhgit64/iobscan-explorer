    ...
    <div class="home_container">
        <div class="home_content_wrap">
                ...
                <div class="home_block_content">
                    ...
                    <ul class="home_block_bottom_content">
                        <div
                            ...
                            :key="index"
                            :class="
                                item.flShowTranslationalAnimation
                                    ? 'animation '
                                    : ''
                            "
                        >
                            <li
                                class="home_block_list_item_content"
                                :class="
                                    item.showAnimation === 'show'
                                        ? 'fadeIn_animation'
                                        : ''
                                "
                            >
                                <p class="home_block_time_content">
                                    ...
                                    <span class="home_age_time">
                                        {{ item.blockAgeTime }}</span
                                    >
                                </p>
                                <p class="home_tx_time_content">
                                    <span class="home_tx"
                                        >{{ $t("ExplorerLang.home.txn") }}:
                                        <span>{{ item.txNumber }}</span></span
                                    >
                                    <span class="home_time">{{
                                        item.time
                                    }}</span>
                                </p>
                            </li>
                ...
                <div class="home_transaction_content">
                    <div class="home_transaction_top_content">
                        <div class="home_transaction_top_title">
                            ...
                            <span>{{
                                $t("ExplorerLang.home.listTransactions")
                            }}</span>
                        </div>
                    ...
                    <ul
                        class="home_transaction_bottom_content"
                        v-if="latestTransaction && latestTransaction.length"
                    >
                        <li
                        ...
                        >
                            ...
                            <p class="home_tx_type_content">
                                <!-- <span class="home_tx_type">{{item.txType}}</span> -->
                                    <!-- :content="setTipDisplay(item.txType)" -->
                                <el-tooltip
                                    :content="formatStr(item.txType)"
                                    placement="top"
                                    :disabled="item.txType.length <= 1"
                                >
                                    <span class="home_tx_type">{{
                                        getDisplayTxType(item.txType)
                                    }}</span>
                                    <span class="message_number" v-if="item.txType.length != 1">+{{item.txType.length - 1}}</span>
                                </el-tooltip>
                                <span class="home_tx_time">{{
                                    item.time
                                }}</span>
                            </p>
                        </li>
                    ...
                    <ul class="home_transaction_bottom_content" v-else>
                        <li style="padding-top: 15px">
                            {{ $t("ExplorerLang.message.noTransaction") }}
                        </li>
                    </ul>
                </div>
            </div>
            <div class="home_proposal_container">
                <div
                    class="home_proposal_container_content"
                    v-for="item in votingPeriodDatas"
    ...
    methods: {
        ...
        formatStr(str) {
            ...
            if (str && Array.isArray(str)) {
                ...
                if (!txType) {
                    txType = sessionStorage.getItem("currentChoiceMsgType")
                        ? sessionStorage.getItem("currentChoiceMsgType")
                        : undefined;
                }
                let msgTxTypeIndex = 0,
                    tmp = str[0];
                ...
                for (let dataKey in data) {
                    ...
                    formatStr += `${
                        this.TX_TYPE_DISPLAY[dataKey] || dataKey
                    } *${data[dataKey] || dataKey},`;
                }
                return formatStr.substring(0, formatStr.length - 1);
            } else {
                return (
                    this.TX_TYPE_DISPLAY[str?.toString()] ||
                    this.TX_TYPE_DISPLAY[str] ||
                    str?.toString() ||
                    str
                );
            }
            ...
            try {
                ...
                if (blockData && blockData.data && blockData.data.length) {
                    ...
                    this.blocksTimer = setInterval(() => {
                        that.latestBlockArray = that.latestBlockArray.map(
                            (item) => {
        ...
        async getTransaction() {
            ...
            try {
                ...
                if (res) {
                    for (
                        let txIndex = 0;
                        txIndex < res.data.length;
                        txIndex++
                    ) {
                        let lastTxTime = Tools.getTimestamp();
                        if (sessionStorage.getItem("lastTxTime")) {
                            lastTxTime = JSON.parse(
                                sessionStorage.getItem("lastTxTime")
                            );
                        }
                    ...
                    sessionStorage.setItem(
                        "lastTxTime",
                        JSON.stringify(Tools.getTimestamp())
                    );
                    this.latestTransaction = res.data.map((item) => {
                        return {
                            flShowTranslationalAnimtation:
                                item.flShowTranslationalAnimation
                                    ? item.flShowTranslationalAnimation
                                    : "",
                            showAnimation: item.showAnimation
                    ...
                    this.transfersTimer = setInterval(() => {
                        this.latestTransaction = this.latestTransaction.map(
                            (item) => {
                                item.txAgeTime = Tools.formatAge(
                                    Tools.getTimestamp(),
                                    item.Time * 1000,
                                    "ago",
                                    ">"
        ...
        showBlockFadeinAnimation(blockList) {
            let storedLastBlockHeight = sessionStorage.getItem(
                "lastBlockHeight"
            )
        ...
        componentAgeTime(beginTime, endTime) {
            return (
                (Number(new Date(beginTime).getTime()) -
                    Number(new Date(endTime).getTime())) /
                1000 /
                100
            ).toFixed(2);
        },
        ...
        async getProposalsByStatus() {
            try {
                let res = await getProposalsListApi(
                    `${proposalStatus.votingPeriod},${proposalStatus.depositPeriod}`
                );
                if (res && res.data && res.data.length > 0) {
                    this.depositPeriodDatas = res.data.filter(
                        (v) => v.status === proposalStatus.depositPeriod
                    );
                    this.depositPeriodDatas = this.depositPeriodDatas.sort(
                        (a, b) => {
                            return b.proposal_id - a.proposal_id;
                        }
                    );
                    this.votingPeriodDatas = res.data.filter(
                        (v) => v.status === proposalStatus.votingPeriod
                    );
                    this.votingPeriodDatas = this.votingPeriodDatas.sort(
                        (a, b) => {
                            return b.proposal_id - a.proposal_id;
                        }
                    );
